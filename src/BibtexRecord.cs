...
public class BibtexRecord
{
    ...
    // TODO: Create a search method where you specify which RecordFields you
    // want searched.

    // Searches the entire record for contained text. Case Insensitive.
    // If the text is found, returns true, else returns false
    public bool SearchRecord(string text)
    {
        for (int i = 0; i < recordFields.Count; i++)
        {
            if (((BibtexRecordField) recordFields[i]).fieldValue != null)
            {
                if (((BibtexRecordField) recordFields[i]).fieldName.ToLower().IndexOf("bibliographer") < 0)
                {
                    if (((BibtexRecordField) recordFields[i]).fieldValue.ToLower().IndexOf(text.ToLower()) > -1)
                        return true;
                }
            }
        }
        return this.IndexContains(text);
    }

    // Searches the entire record for contained text. Case Insensitive.
    // If the text is found, returns true, else returns false
    public bool SearchField(string field, string text)
    {
        for (int i = 0; i < recordFields.Count; i++)
        {
            if ((((BibtexRecordField) recordFields[i]).fieldName).ToLower() == field.ToLower())
            {
                if (((BibtexRecordField) recordFields[i]).fieldName.ToLower().IndexOf("bibliographer") < 0)
                {
                    if (((BibtexRecordField) recordFields[i]).fieldValue.ToLower().IndexOf(text.ToLower()) > -1)
